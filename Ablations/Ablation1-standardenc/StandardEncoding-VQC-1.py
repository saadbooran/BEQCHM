...
CONDITION_NAME = "Ablation_Standard_Encoding_VQC_1"
...
def standard_angle_encoding(x, order=1):
...
def enhanced_quantum_circuit(inputs, weights):
    ...
    for i in range(n_qubits):
        if i < len(inputs):
            ...
            feature_val_tensor = standard_angle_encoding(inputs[i])
...
class singleQuantumGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=112, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        ...
        self.quantum_circuit = EnhancedQuantumLayer(n_qubits)
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"ABLATION (Standard Encoding): 1 Quantum Circuit, standard angle encoding (4 time points) + GRU (136 time points, 112 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 112  
        output_dim = 2
        model = singleQuantumGRUModel(input_size=input_size, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "ablation_standard_encoding_vqc1_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_ablation_standard_encoding_vqc1_metrics.csv")
    ...
    print("- ABLATION (Standard Encoding): 1 Quantum Circuit (standard angle encoding, no Bessel) + GRU")
    print("- Quantum: 4 time points with STANDARD (non-Bessel) angle encoding [ablation]")
