...
CONDITION_NAME = "Condition_1_Classical_Replacement"  
...
class ClassicalGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=112, output_dim=2, num_layers=NUM_LAYERS):
        super().__init__()
        ...
        self.classical_extractor = ClassicalFeatureExtractor(input_dim=4, output_dim=10)
        
        
        classical_output_dim = 10
    ...
    def forward(self, x):
        ...
        classical_out = self.classical_extractor(x)  
        ...
        combined = torch.cat([gru_last, classical_out], dim=1) 
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CLASSICAL MODEL: Classical Feature Extractor (4 time points) + GRU (136 time points, 112 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 112  
        output_dim = 2
        model = ClassicalGRUModel(input_size=input_size, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_1_classical_replacement_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_1_classical_metrics.csv")
    ...
    print("- CLASSICAL REPLACEMENT: Classical Feature Extractor + GRU")
    print("- Classical: 4 time points with neural network encoding")
