...
CONDITION_NAME = "Condition_2_Classical_Replacement"  
...
class Classical2GRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=96, output_dim=2, num_layers=NUM_LAYERS):
        super().__init__()
        ...
        classical_output_dim = 20
        
        
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size + classical_output_dim, hidden_size),
            nn.ReLU(),
            nn.Dropout(DROPOUT),
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CLASSICAL MODEL: 2 Classical Feature Extractors (8 time points) + GRU (132 time points, 96 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 96  
        output_dim = 2
        model = Classical2GRUModel(input_size=input_size, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_2_classical_replacement_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_2_classical_metrics.csv")
    ...
    print("- CLASSICAL REPLACEMENT: 2 Classical Feature Extractors + GRU")
    ...
    print("- Combined features from both classical extractors and GRU for classification")
