...
CONDITION_NAME = "Condition_3_Classical_Replacement" 
...
class ClassicalFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        if x_processed.shape[1] < self.input_dim:
            padding = torch.zeros(x_processed.shape[0], self.input_dim - x_processed.shape[1], 
                                device=x_processed.device)
            x_processed = torch.cat([x_processed, padding], dim=1)
        elif x_processed.shape[1] > self.input_dim:
            x_processed = x_processed[:, :self.input_dim]
        
        
        features = self.feature_extractor(x_processed)
        return features


class Classical3GRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=80, output_dim=2, num_layers=NUM_LAYERS):
        super().__init__()
        ...
        self.classical_extractor1 = ClassicalFeatureExtractor(input_dim=4, output_dim=10)  
        self.classical_extractor2 = ClassicalFeatureExtractor(input_dim=4, output_dim=10)  
        self.classical_extractor3 = ClassicalFeatureExtractor(input_dim=4, output_dim=10)  
        
        
        classical_output_dim = 30
        
        
        self.classifier = nn.Sequential(
            nn.Linear(hidden_size + classical_output_dim, 64),  
            nn.ReLU(),
            nn.Dropout(DROPOUT),
            nn.Linear(64, 32),  
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        c_out1 = self.classical_extractor1(x[:, :4, :])  
        
        
        c_out2 = self.classical_extractor2(x[:, 4:8, :])  
        
        
        c_out3 = self.classical_extractor3(x[:, 8:12, :])  
        
        
        c_out = torch.cat([c_out1, c_out2, c_out3], dim=1) 
        ...
        combined = torch.cat([gru_last, c_out], dim=1)  
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CLASSICAL MODEL: 3 Classical Feature Extractors (12 time points) + GRU (128 time points, 80 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 80 
        output_dim = 2
        model = Classical3GRUModel(input_size=input_size, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_3_classical_replacement_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_3_classical_metrics.csv")
    ...
    print("- CLASSICAL REPLACEMENT: 3 Classical Feature Extractors + GRU")
    print("- Classical Extractor 1: 4 time points (1-4) with neural network encoding")
    print("- Classical Extractor 2: 4 time points (5-8) with neural network encoding")
    print("- Classical Extractor 3: 4 time points (9-12) with neural network encoding")
    print("- GRU: 128 time points with 80 hidden size")
    print("- Combined features from all three classical extractors and GRU for classification")
