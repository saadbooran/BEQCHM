...
CONDITION_NAME = "Condition_1_Classical_Bessel_1_Block"
...
class ClassicalBesselGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=112, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        ...
        self.bessel_block = ClassicalBesselLayer(n_features=n_qubits, out_dim=10)
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 1 (Classical Bessel ablation): 1 Classical-Bessel block (4 time points) + GRU (136 time points, 112 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 112
        output_dim = 2
        model = ClassicalBesselGRUModel(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_1_classical_bessel_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_1_classical_bessel_metrics.csv")
    ...
    print("- CONDITION 1 (Classical Bessel ablation): 1 Classical-Bessel block + GRU")
    print("- Classical-Bessel: 4 time points with Bessel encoding -> FC(4->16->10)")
