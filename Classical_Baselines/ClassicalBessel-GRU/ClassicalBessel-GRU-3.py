...
CONDITION_NAME = "Condition_3_Classical_Bessel_3_Blocks"
...
class ClassicalBesselLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_bessel = bessel_encoding(x_b)
        if not torch.is_tensor(x_bessel):
            x_bessel = torch.tensor(x_bessel, dtype=torch.float32, device=x.device)
        x_bessel = x_bessel.to(x.device).float()

        out = self.net(x_bessel)
        return out


class ClassicalBesselGRUModel3(nn.Module):
    def __init__(self, input_size, hidden_size=80, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        ...
        self.bessel_block1 = ClassicalBesselLayer(n_features=n_qubits, out_dim=10)
        self.bessel_block2 = ClassicalBesselLayer(n_features=n_qubits, out_dim=10)
        self.bessel_block3 = ClassicalBesselLayer(n_features=n_qubits, out_dim=10)
    ...
    def forward(self, x):
        ...
        b_out1 = self.bessel_block1(x[:, :4, :])   
        b_out2 = self.bessel_block2(x[:, 4:8, :])   
        b_out3 = self.bessel_block3(x[:, 8:12, :]) 
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 3 (Classical Bessel ablation): 3 Classical-Bessel blocks (12 time points) + GRU (128 time points, 80 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 80
        output_dim = 2
        model = ClassicalBesselGRUModel3(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_3_classical_bessel_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_3_classical_bessel_metrics.csv")
    ...
    print("- CONDITION 3 (Classical Bessel ablation): 3 Classical-Bessel blocks + GRU")
    print("- Classical-Bessel: 3x [4 time points with Bessel encoding -> FC(4->16->10)]")
