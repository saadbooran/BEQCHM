...
CONDITION_NAME = "Baseline_Classical_Transformer"
...
class TransformerModel(nn.Module):
    def __init__(self, input_size=1, d_model=32, nhead=4, num_layers=2,
                 dim_feedforward=64, output_dim=2, seq_len=140):
        super().__init__()
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"BASELINE: Classical Transformer on full {SEQ_LEN}-time-point signal (no quantum component)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        model = TransformerModel(input_size=FEATURES, output_dim=2, seq_len=SEQ_LEN).to(DEVICE)
...
if __name__ == "__main__":
    ...
    _tmp_model = TransformerModel(input_size=1, output_dim=2, seq_len=140)
    _n_params = count_params(_tmp_model)
    print(f"\n{'='*70}")
    print(f"Transformer Model parameters: {_n_params}")
    print(f"{'='*70}\n")
    del _tmp_model
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "baseline_classical_transformer_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_baseline_classical_transformer_metrics.csv")
    ...
    print("- BASELINE: Encoder-only Transformer (no quantum component)")
    print("- 2 encoder layers, d_model=32, 4 attention heads, dim_feedforward=64")
    print("- Sinusoidal positional encoding, mean pooling -> FC classifier (32 -> 2)")
