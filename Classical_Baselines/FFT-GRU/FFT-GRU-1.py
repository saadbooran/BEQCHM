...
CONDITION_NAME = "Condition_1_Classical_FFT_1_Block"
...
class ClassicalFFTGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=112, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        ...
        self.fft_block = ClassicalFFTLayer(n_features=n_qubits, out_dim=10)
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 1 (Classical FFT ablation): 1 block Classical-FFT block(s) (4 time points) + GRU (136 time points, 112 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 112
        output_dim = 2
        model = ClassicalFFTGRUModel(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_1_classical_fft_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_1_classical_fft_metrics.csv")
    ...
    print("- CONDITION 1 (Classical FFT ablation): 1 block Classical-FFT block(s) + GRU")
    print("- Classical-FFT: 4 time points with FFT transform -> FC(->16->10)")
