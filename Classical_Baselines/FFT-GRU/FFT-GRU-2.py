...
CONDITION_NAME = "Condition_2_Classical_FFT_2_Blocks"
...
class ClassicalFFTGRUModel2(nn.Module):
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 2 (Classical FFT ablation): 2 Classical-FFT blocks (8 time points) + GRU (132 time points, 96 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 96
        output_dim = 2
        model = ClassicalFFTGRUModel2(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_2_classical_fft_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_2_classical_fft_metrics.csv")
    ...
    print("- CONDITION 2 (Classical FFT ablation): 2 Classical-FFT blocks + GRU")
    print("- Classical-FFT: 2x [4 time points with FFT transform -> FC(4->16->10)]")
