...
CONDITION_NAME = "LSTM_Only_Results"  
...
class LSTMOnlyModel(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.classifier(lstm_last)
        return logits
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"LSTM ONLY MODEL: Processing ALL {SEQ_LEN} time points through LSTM (no quantum)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "lstm_only_140ts_results.csv")  
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_lstm_only_metrics.csv")  
    ...
    print("- LSTM ONLY MODEL (NO QUANTUM COMPONENTS)")  
    print("- Processing ALL 140 time points through LSTM")  
