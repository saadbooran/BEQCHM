...
HIDDEN_SIZES = {1: 128, 2: 110, 3: 86}
TARGET_BEQCHM_PARAMS = {1: 58822, 2: 44250, 3: 28990}

if CONDITION not in HIDDEN_SIZES:
    raise ValueError("CONDITION must be 1, 2, or 3")

HIDDEN_SIZE = HIDDEN_SIZES[CONDITION]
...
CONDITION_NAME = f"ParamMatched_GRU_Condition_{CONDITION}"
...
print(f"Hidden size: {HIDDEN_SIZE} (target BEQCHM-{CONDITION} params: {TARGET_BEQCHM_PARAMS[CONDITION]})")
...
class ParamMatchedGRUModel(nn.Module):
    def __init__(self, hidden_size, input_size=1, output_dim=2):
        super().__init__()
...
def plot_training_curves(train_losses, test_losses, train_accuracies, test_accuracies,
    ...
    ax3.set_xlabel('Epoch'); ax3.set_ylabel('Balanced Accuracy'); ax3.legend(); ax3.grid(True, alpha=0.3)
    ...
    ax4.set_xlabel('Epoch'); ax4.set_ylabel('Metric Value'); ax4.legend(); ax4.grid(True, alpha=0.3)
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"BASELINE: Parameter-matched classical GRU (hidden={HIDDEN_SIZE}), Condition {CONDITION}")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        model = ParamMatchedGRUModel(hidden_size=HIDDEN_SIZE, input_size=FEATURES, output_dim=2).to(DEVICE)

        if roi_number == extract_roi_number(os.path.basename(file_path)) and fold == 1:
            n_params = count_params(model)
            print(f"*** Model parameter count: {n_params} (target BEQCHM-{CONDITION}: {TARGET_BEQCHM_PARAMS[CONDITION]}) ***")
...
if __name__ == "__main__":
    ...
    _tmp_model = ParamMatchedGRUModel(hidden_size=HIDDEN_SIZE, input_size=1, output_dim=2)
    _n_params = count_params(_tmp_model)
    print(f"\n{'='*70}")
    print(f"CONDITION {CONDITION}: hidden_size={HIDDEN_SIZE}")
    print(f"Model parameters: {_n_params}  |  Target (BEQCHM-{CONDITION}): {TARGET_BEQCHM_PARAMS[CONDITION]}  |  Diff: {_n_params - TARGET_BEQCHM_PARAMS[CONDITION]}")
    print(f"{'='*70}\n")
    del _tmp_model
    ...
    for idx, fpath in enumerate(csv_files_sorted, 1):
        ...
        try:
            ...
            all_results.append({
                'ROI_Number': roi_number, 'Avg_Balanced_Accuracy': 0.0,
                'Fold_1_Balanced_Accuracy': 0.0, 'Fold_2_Balanced_Accuracy': 0.0,
                'Fold_3_Balanced_Accuracy': 0.0, 'Std_Balanced_Accuracy': 0.0,
                'Status': f"Error: {str(e)}", 'Filename': filename
            })

    try:
        results_df = pd.DataFrame(all_results)
        results_path = os.path.join(CONDITION_FOLDER, f"param_matched_gru_condition_{CONDITION}_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, f"detailed_param_matched_gru_condition_{CONDITION}_metrics.csv")
    ...
    if not results_df.empty:
        ...
        if not successful_runs.empty:
            print(f"\n=== Overall Statistics (Condition {CONDITION}, hidden={HIDDEN_SIZE}) ===")
    ...
    print(f"\n=== Analysis Complete ===")
    print(f"- BASELINE: Parameter-matched classical GRU (no quantum component)")
    print(f"- Condition {CONDITION}: hidden_size={HIDDEN_SIZE}, matched to BEQCHM-{CONDITION} ({TARGET_BEQCHM_PARAMS[CONDITION]} params)")
