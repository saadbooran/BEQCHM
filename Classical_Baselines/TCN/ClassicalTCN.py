...
CONDITION_NAME = "Baseline_Classical_TCN"
...
class TCNModel(nn.Module):
    ...
    def __init__(self, input_channels=1, num_channels=(32, 64, 96, 128),
                 kernel_size=3, output_dim=2):
        super().__init__()
        layers = []
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"BASELINE: Classical TCN on full {SEQ_LEN}-time-point signal (no quantum component)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        model = TCNModel(input_channels=FEATURES, output_dim=2).to(DEVICE)
...
if __name__ == "__main__":
    ...
    _tmp_model = TCNModel(input_channels=1, output_dim=2)
    _n_params = count_params(_tmp_model)
    print(f"\n{'='*70}")
    print(f"TCN Model parameters: {_n_params}")
    print(f"{'='*70}\n")
    del _tmp_model
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "baseline_classical_tcn_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_baseline_classical_tcn_metrics.csv")
    ...
    print("- BASELINE: Temporal Convolutional Network (no quantum component)")
    print("- 4 dilated causal conv blocks (32/64/96/128 channels, dilation 1/2/4/8)")
    print("- Global Average Pooling -> FC classifier (64 -> 2)")
