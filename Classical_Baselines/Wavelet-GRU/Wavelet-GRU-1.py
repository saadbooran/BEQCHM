...
CONDITION_NAME = "Condition_1_Classical_WAVELET_1_Block"
...
class ClassicalWaveletGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size=112, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        super().__init__()
        ...
        self.wavelet_block = ClassicalWaveletLayer(n_features=n_qubits, out_dim=10)

        quantum_output_dim = 10 
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 1 (Classical Wavelet ablation): 1 block Classical-Wavelet block(s) (4 time points) + GRU (136 time points, 112 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 112
        output_dim = 2
        model = ClassicalWaveletGRUModel(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_1_classical_wavelet_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_1_classical_wavelet_metrics.csv")
    ...
    print("- CONDITION 1 (Classical Wavelet ablation): 1 block Classical-Wavelet block(s) + GRU")
    print("- Classical-Wavelet: 4 time points with Wavelet transform -> FC(->16->10)")
    print("- GRU: 136 time points with 112 hidden size")
