...
CONDITION_NAME = "Condition_2_Classical_WAVELET_2_Blocks"
...
class ClassicalWaveletGRUModel2(nn.Module):
    ...
    def forward(self, x):
        ...
        b_out = torch.cat([b_out1, b_out2], dim=1)  
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 2 (Classical Wavelet ablation): 2 Classical-Wavelet blocks (8 time points) + GRU (132 time points, 96 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 96
        output_dim = 2
        model = ClassicalWaveletGRUModel2(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_2_classical_wavelet_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_2_classical_wavelet_metrics.csv")
    ...
    print("- CONDITION 2 (Classical Wavelet ablation): 2 Classical-Wavelet blocks + GRU")
    print("- Classical-Wavelet: 2x [4 time points with Wavelet transform -> FC(4->16->10)]")
