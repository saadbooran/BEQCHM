...
CONDITION_NAME = "Condition_3_Classical_WAVELET_3_Blocks"
...
def wavelet_encoding(x, wavelet='db1'):
    ...
    flat = x_np.reshape(-1, n_features)
    ...
    if coeffs.shape[-1] < n_features:
        coeffs = np.pad(coeffs, ((0, 0), (0, n_features - coeffs.shape[-1])), mode='constant')
    else:
        coeffs = coeffs[:, :n_features]

    coeffs = coeffs.reshape(orig_shape + (n_features,))

    if return_tensor:
        return torch.tensor(coeffs, dtype=torch.float32, device=original_device)
    return coeffs.astype(np.float32)

class ClassicalWaveletLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_transformed = wavelet_encoding(x_b)
...
class ClassicalWaveletGRUModel3(nn.Module):
    def __init__(self, input_size, hidden_size=80, output_dim=2, num_layers=NUM_LAYERS, n_qubits=4):
        ...
        self.wavelet_block1 = ClassicalWaveletLayer(n_features=n_qubits, out_dim=10)
        self.wavelet_block2 = ClassicalWaveletLayer(n_features=n_qubits, out_dim=10)
        self.wavelet_block3 = ClassicalWaveletLayer(n_features=n_qubits, out_dim=10)
    ...
    def forward(self, x):
        ...
        b_out1 = self.wavelet_block1(x[:, :4, :])    
        b_out2 = self.wavelet_block2(x[:, 4:8, :])   
        b_out3 = self.wavelet_block3(x[:, 8:12, :])  
...
def run_roi_file(file_path, roi_number, n_splits=3):
    ...
    print(f"CONDITION 3 (Classical Wavelet ablation): 3 Classical-Wavelet blocks (12 time points) + GRU (128 time points, 80 hidden)")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
        ...
        hidden_size = 80
        output_dim = 2
        model = ClassicalWaveletGRUModel3(input_size=input_size,
...
if __name__ == "__main__":
    ...
    try:
        ...
        results_path = os.path.join(CONDITION_FOLDER, "condition_3_classical_wavelet_results.csv")
    ...
    try:
        if all_detailed_metrics:
            ...
            detailed_path = os.path.join(CONDITION_FOLDER, "detailed_condition_3_classical_wavelet_metrics.csv")
    ...
    print("- CONDITION 3 (Classical Wavelet ablation): 3 Classical-Wavelet blocks + GRU")
    print("- Classical-Wavelet: 3x [4 time points with Wavelet transform -> FC(4->16->10)]")
